...
const tastiest = recipe => {
    let recipeIndex = Math.max(...recipe.map(ingredients => ingredients.veg.length))
    return `The recipe at index ${recipe.map(ingredients => ingredients.veg.length).indexOf(recipeIndex)} is the tastiest!!!`;
}